...
<table class='table table-bordered' style="/*min-width:1170px;*/min-width:100%" id="table_input">
    <thead>
        <tr>
            ...
            <th rowspan=2 style="width:180px;">សាខា / ការិយាល័យ</th>
            <th colspan=5 >២.១ ទំនិញនាំចេញជាប់អាករ / មិនជាប់អាករ</th>
	...
	$itm = $this->m_global->select_ExportRevenueSum(TBLITEMSEXPORT_REVENUES,array("branch_code"=>$branch->code,"year"=>$year,"month"=>$month, 'isbur'=>0),NULL,'branch_code');
    $oth_tax = $this->m_global->select_ExportRevenueSum(TBLITEMSEXPORT_REVENUES,array("branch_code"=>$branch->code,"year"=>$year,"month"=>$month, 'isbur'=>2),NULL,'branch_code');
	...
	$count_qty_2 = $count_qty_2 + (!$itm==null?$itm[0]->qty_2:0);
	$count_tax_base_2 = $count_tax_base_2 + (!$itm==null?$itm[0]->tax_base_2:0);
	$count_tax_amount_2 = $count_tax_amount_2 + (!$itm==null?$itm[0]->tax_amount_2:0);
	
	?>
		<tr class="">
			<td  class="td_delete"><?php echo "$i"; ?></td>
            <td class="td_i"><span class="enum"><?php echo($branch->name_print); ?></span></td>
            <td class="td_qty"><span class="disp"><?php echo num_format(!$itm==null?$itm[0]->qty:0); ?></span></td>
            ...
            <td class="td_qty_2"><span class="disp"><?php echo num_format(!$itm==null?$itm[0]->qty_2:0); ?></span></td>
            <td class="td_tax_base_2"><span class="disp"><?php echo num_format(!$itm==null?$itm[0]->tax_base_2:0); ?></span></td>
            <td class="td_tax_amount_2"><span class="disp"><?php echo num_format(!$itm==null?$itm[0]->tax_amount_2:0); ?></span></td>
        </tr>
    <?php
		$offices = $this->m_customs->getOffices($branch->code);
		if(count($offices)>0){
			foreach($offices as $office){
				$sub = $this->m_global->select_ExportRevenueSum(TBLITEMSEXPORT_REVENUES,array("office_code"=>$office->code,"year"=>$year,"month"=>$month, 'isbur'=>0),NULL,'office_code');
				$sub3 = $this->m_global->select_ExportRevenueSum(TBLITEMSEXPORT_REVENUES,array("office_code"=>$office->code,"year"=>$year,"month"=>$month, 'isbur'=>2),NULL,'office_code');
                ...
                <tr class="">
                    ...
                    <td class="td_i_sub"><span class="enum">- <?php echo($office->name_print); ?></span></td>
                    ...
                    <td class="td_qty_2_sub"><span class="disp"><?php echo num_format(!$sub==null?$sub[0]->qty_2:0); ?></span></td>
                    <td class="td_tax_base_2_sub"><span class="disp"><?php echo num_format(!$sub==null?$sub[0]->tax_base_2:0); ?></span></td>
                    <td class="td_tax_amount_2_sub"><span class="disp"><?php echo num_format(!$sub==null?$sub[0]->tax_amount_2:0); ?></span></td>
                </tr>
            <?php
				}
			}
...
$(document).ready(function(e) {
	...
	$(document).on('change','#select-month',function(e){
		if($(this).val()!='0' && $('select-year').val()!='0'){
			window.location = '<?php echo base_url('admin/monthly_export_office_revenues') ?>'+'?'+'m=' + $(this).val()+'&y='+$('#select-year').val();
			}
		});
	$(document).on('change','#select-year',function(e){
		if($(this).val()!='0' && $('select-month').val()!='0'){
			window.location = '<?php echo base_url('admin/monthly_export_office_revenues') ?>'+'?'+'y=' + $(this).val()+'&m='+$('#select-month').val();
			}
		});
});

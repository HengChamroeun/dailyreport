...
$branch_name = $this->m_global->select_record(TBLOFFICES,array('code'=>$branch),'name');
$office_name = $this->m_global->select_record(TBLOFFICES,array('code'=>$office),'name');
$office_count = $this->m_global->count_row(TBLOFFICES,array('parent_code'=>$branch,'status'=>'1'));
//echo "$branch_name";
//echo "$office_name";
//echo "$office_count";
?>
...
<h3>សំរាប់ 
<?php
if($branch!='0' && $office=='0'){echo($branch_name);}
if($office!='0'){echo($office_name);}
?>
</h3>
<br/>
<?php
...
<table class='table table-bordered table-striped ' id="table_input">
    ...
    $tax_oth_total = $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("year"=>$year,"month"=>$month,"isbur"=>2),array('item_id'=>'ASC'),'item_id');
	if($office_count == 0){
		$item_revenues =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("office_code"=>$branch,"year"=>$year,"month"=>$month,"isbur"=>0),array('item_id'=>'ASC'),'item_id');
        $tax_oth_total =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("office_code"=>$branch,"year"=>$year,"month"=>$month,"isbur"=>2),array('item_id'=>'ASC'),'item_id');

	}
	if($office_count > 0 && $office == '0'){
		$item_revenues =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("branch_code"=>$branch,"year"=>$year,"month"=>$month,"isbur"=>0),array('item_id'=>'ASC'),'item_id');
        $tax_oth_total =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("branch_code"=>$branch,"year"=>$year,"month"=>$month,"isbur"=>2),array('item_id'=>'ASC'),'item_id');
	}
	if($office != '0'){
		$item_revenues =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("branch_code"=>$branch,"office_code"=>$office,"year"=>$year,"month"=>$month,"isbur"=>0),array('item_id'=>'ASC'),'item_id');
        $tax_oth_total =  $this->m_global->select_ItemRevenueSum(TBLITEM_REVENUES,array("branch_code"=>$branch,"office_code"=>$office,"year"=>$year,"month"=>$month,"isbur"=>2),array('item_id'=>'ASC'),'item_id');
	}
	//echo $this->db->last_query();
    foreach ($tax_oth_total as $row) { $oth_total_tmp = $row->tax_amount; }
    ...
    <input type="hidden" id="tax_oth_total" value="<?= $oth_total_tmp ?>">
...
$(document).ready(function(e) {
	...
	$(document).on('change','#select-month',function(e){
		if($(this).val()!='0' && $('select-year').val()!='0'){
			window.location = '<?php echo base_url('admin/monthly_item_revenues_by_office') ?>'+'?'+'m=' + $(this).val()+'&y='+$('#select-year').val()+'&branch=<?php echo($branch); ?>'+'&office=<?php echo($office); ?>';
			}
		});
	$(document).on('change','#select-year',function(e){
		if($(this).val()!='0' && $('select-month').val()!='0'){
			window.location = '<?php echo base_url('admin/monthly_item_revenues_by_office') ?>'+'?'+'y=' + $(this).val()+'&m='+$('#select-month').val()+'&branch=<?php echo($branch); ?>'+'&office=<?php echo($office); ?>';
			}
		});
});
